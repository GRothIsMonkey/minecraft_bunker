...
public final class Level4 {
    ...
    private void spine() {
        ...
        k.sign(-330, F + 5, 144, Dir.NORTH, "§lHANGAR", "Vehicle bay", "Freight lift", "");
    ...
    private void hangar() {
        ...
        k.sign(-306, F + 3, 165, Dir.SOUTH, "§eFREIGHT LIFT", "Stand on pad", "use [Lift Up]", "");
        // vehicles
        apc(-338, 154);
        truck(-338, 166);
        helicopter(-322, 164);
        ...
        k.sign(-300, F + 3, 172, Dir.WEST, "§4FUEL POINT", "No smoking", "", "");
    ...
    private void storageHall() {
        ...
        k.sign(-402, F + 4, 146, Dir.SOUTH, "§lMAIN STORES", "Categorised", "storage", "§8label = sign");
    ...
    /** Hopper-fed 12 furnace super smelter: load ores/fuel into the two top chests, collect from the end chest. */
    private void smelter() {
        int x1 = -382, x2 = -367, z1 = 154, z2 = 184;
        int zf = 160, xs = -380, xe = -369;
        for (int x = xs; x <= xe; x++) {
            boolean last = x == xe;
            k.set(x, F + 1, zf, B.hopper(Dir.EAST));                // output chain
            k.set(x, F + 2, zf, B.furnace(Dir.SOUTH));
            k.set(x, F + 3, zf, B.hopper(Dir.DOWN));                 // input hopper
            k.set(x, F + 4, zf, B.hopper(last ? Dir.DOWN : Dir.EAST)); // input distribution chain
            k.set(x, F + 2, zf - 1, B.hopper(Dir.SOUTH));           // fuel hopper into furnace back
            k.set(x, F + 3, zf - 1, B.hopper(last ? Dir.DOWN : Dir.EAST)); // fuel distribution chain
            k.set(x, F + 1, zf - 1, B.SB);
            k.set(x, F + 1, zf + 1, B.slab(B.SLAB_STONE, false));
        }
        k.chest(xs, F + 5, zf, Dir.SOUTH);                          // ore loading chest
        k.chest(xs, F + 4, zf - 1, Dir.SOUTH, Loot.smelterFuel());  // fuel loading chest
        ...
        // backing wall for the fuel line
        k.fill(xs - 1, F + 1, zf - 2, xe + 1, F + 5, zf - 2, B.SB);
        k.fill(xs - 1, F + 1, zf - 1, xs - 1, F + 2, zf - 1, B.SB);
        k.set(xs - 1, F + 3, zf - 1, B.slab(B.SLAB_STONEBRICK, false));
        k.sign(xs - 1, F + 5, zf, Dir.WEST, "§lORE INPUT", "chest above", "first furnace", "");
        k.sign(xs - 1, F + 4, zf - 1, Dir.WEST, "§lFUEL INPUT", "coal/charcoal", "", "");
    ...
    private void machineRoom() {
        ...
        for (int x = x1 + 2; x <= x2 - 2; x += 4) {
            k.set(x, F + 1, 104, B.litFurnace(Dir.SOUTH));
            k.set(x, F + 2, 104, B.IRON);
